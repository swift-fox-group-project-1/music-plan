...
function getQR(params) {
  Swal.fire({
    ...
    html: `<a href="" onclick="sendmail(event, params)">Send To Your Email</a>`,
    focusConfirm: false,
    showCancelButton: true,
  })
}

function sendmail(event, obj) {
  event.preventDefault()
  console.log(event);
  event.preventDefault();
  $.ajax(`${endpoint}/login`, {
  ...
  .fail(err => console.log(err))
  .always(() => console.log('email sent'));

  Swal.fire({
    title: 'Sweet!',
    text: 'Modal with a custom image.',
    imageAlt: 'Custom image',
    html: `<a href="" onclick="sendmail(event, obj)">Send To Your Email</a>`,
    focusConfirm: false,
    showCancelButton: true,
  })
}
